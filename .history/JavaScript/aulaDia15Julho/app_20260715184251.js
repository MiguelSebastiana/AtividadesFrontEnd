...
/* 
    ...
    No arquivo JS, Nós criamos objetos de forma livre. As chaves (propiedades) não precisam de aspas, e podemos usar
    tanto aspas simples quanto aspas duplas nos textos.

    Como a Api nos envia (JSON)

    O Json nao é um codigo js ativo, ele é puramente um formato de texto. Como ele precisa ser lido pelo JS, eçe exoge regras
    muito mais rigidas e formais do que o Objeto literal

    Regra de ouro aqui:

    - As chaves devem ter aspas duplas: No Js escrevemos nome:, mas no JSON somos obrigados a escrever "nome":. Se esquecer as 
    aspas nas chaves, a API vai quebrar.
    - Apenas aspas duplas (" ") são aceitas. No JSON o uso de aspas simples (' ') nos textos gera um erro de sintaxe. Tudo o que
    for texto precisa estar entre aspas duplas (" ").
    - Sem funções ou metodos: Um objeto JS pode ter funções dentro dele (como o metodo apresentar()). O JSON nao aceita funções,
    ele guarda apenas dados brutos (texto, numeros, booleans, arrays...)
*/ 
